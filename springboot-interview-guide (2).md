...
In plain Java, you own the object's entire life:
```java
UserService service = new UserService(); // you create it, you manage it, you pass its dependencies
```

In Spring, you just declare it:
```java
@Service
public class UserService { }
```

Spring creates it, wires its dependencies, initializes it, and destroys it at shutdown. That managed object is a **bean**.

> **Definition:** A Spring Bean is any object whose lifecycle (creation -> dependency injection -> initialization -> destruction) is managed by the Spring IoC container instead of your own code.

### 2.2 Two ways a bean gets created

**Method 1 -- Component scanning (implicit).** You annotate your own class, Spring finds it during startup and instantiates it:
```java
@Service   // or @Component, @Repository, @Controller
public class UserService { }
```

**Method 2 -- @Bean method (explicit).** You write the creation logic yourself inside a @Configuration class -- essential for third-party classes you don't own and can't annotate:
```java
@Configuration
...
**@Component vs @Bean -- the actual distinction interviewers want:**

| | @Component (+ stereotypes) | @Bean |
|---|---|---|
| Who instantiates | Spring, automatically, via classpath scan | You, explicitly, inside the method body |
| Use case | Your own classes | Third-party / library classes you can't annotate (ObjectMapper, RestTemplate, KafkaTemplate) |
| Control over construction | Limited (constructor injection only) | Full -- any custom logic before returning the object |

### 2.3 Bean Lifecycle -- full flow

```mermaid
flowchart TD
    A["Application starts"] --> B["1. Instantiate bean<br/>(constructor call)"]
    B --> C["2. Inject dependencies<br/>(field/setter DI happens here)"]
    C --> D["3. Aware interfaces called<br/>(BeanNameAware, ApplicationContextAware...)"]
    D --> E["4. BeanPostProcessor.postProcessBeforeInitialization()"]
    E --> F["5. @PostConstruct /<br/>InitializingBean.afterPropertiesSet()"]
    F --> G["6. Custom init-method"]
    G --> H["7. BeanPostProcessor.postProcessAfterInitialization()<br/>(AOP proxies wrapped here)"]
    H --> I["Bean ready -- application running"]
    I --> J["Container shutdown triggered"]
    J --> K["8. @PreDestroy /<br/>DisposableBean.destroy()"]
    K --> L["9. Custom destroy-method"]
```
...
public class CacheWarmer {
    ...
    @PostConstruct
    public void init() {
        System.out.println("Warm Redis cache");
    }
...
Application starts
  -> Create CacheWarmer
  -> Inject dependencies
  -> @PostConstruct runs: "Warm Redis cache"
Application running...
Application stops
  -> @PreDestroy runs: "Closing resources"
```
...
| Scope | Meaning |
|---|---|
| singleton (default) | One instance shared by the entire application context |
...
flowchart LR
    subgraph Singleton["singleton (default)"]
        S["UserService<br/>(one instance)"] --- CA["Controller A"]
        S --- CB["Controller B"]
    end
    subgraph Prototype["prototype"]
        PA["Controller A"] --> P1["Bean instance 1"]
        PB["Controller B"] --> P2["Bean instance 2"]
    end
```

### 2.5 Why singleton beans must be stateless

Because there's only one instance, every concurrent request runs through the same object. If you store mutable state in an instance field:
```java
@Service
...
```
User A, User B, and User C calling increment() concurrently are all mutating the same count variable on the same object -- a classic race condition (lost updates, inconsistent reads).

**Fix -- use local (method) variables, not instance fields, for request-specific data:**
```java
@Service
public class OrderService {
    public void process(User user) {
        String name = user.getName();   // lives on this thread's stack -- safe
    }
}
```
Local variables are stack-allocated per method call/thread, so there's no sharing across concurrent requests. This is the reason "singleton beans should be stateless" is a rule, not just convention.

### 2.6 Constructor vs @PostConstruct -- ordering matters

```java
@Service
public class OrderService {
    @Autowired
    PaymentService paymentService;   // not yet available inside the constructor below

    public OrderService() {
        // paymentService is still null here (field injection happens after construction)
    }

    @PostConstruct
    public void init() {
        // paymentService is guaranteed to be injected by this point
    }
}
```
Order: **Constructor runs -> dependencies injected -> @PostConstruct runs.** Any initialization logic that needs an injected dependency belongs in @PostConstruct, never in the constructor (for field/setter injection) -- the dependency simply isn't there yet.

### 2.7 BeanPostProcessor -- how @Transactional actually works

Spring sometimes doesn't hand you your raw object -- it intercepts bean creation and wraps it:
```mermaid
flowchart LR
    A["OrderService created"] --> B["BeanPostProcessor<br/>inspects the bean"]
    B --> C{"Has @Transactional?"}
    C -->|yes| D["Wrap in a Proxy"]
    C -->|no| E["Return bean as-is"]
    D --> F["Caller holds a reference<br/>to the Proxy, not raw OrderService"]
```
```java
@Service
public class OrderService {
...
sequenceDiagram
    ...
    Caller->>Proxy: saveOrder()
    Proxy->>Proxy: begin transaction
    Proxy->>Real: actual saveOrder() logic
    Real-->>Proxy: return
    Proxy->>Proxy: commit / rollback
    Proxy-->>Caller: return
```
This is exactly why self-invocation breaks @Transactional (covered in topic 9) -- calling this.saveOrder() from inside the same class skips the proxy entirely, since 'this' refers to the raw object, not the wrapped one. BeanPostProcessor is the general-purpose extension point Spring uses internally to build these proxies for @Transactional, @Async, @Cacheable, and @Scheduled.

**Why interviewers ask:** Tests whether you understand what Spring is actually doing behind @Component -- and whether you know why proxies exist, which is the foundation for half the "gotcha" questions on @Transactional/@Cacheable later.

**Q&A:**
- **Q: What is a Spring Bean?**
  A: An object whose lifecycle -- creation, dependency injection, initialization, destruction -- is managed by the Spring IoC container instead of your own code.
- **Q: @Component vs @Bean -- when would you use each?**
  A: @Component for your own classes, discovered via classpath scanning. @Bean for objects you don't own the source of (third-party library classes) or that need custom construction logic you can't express with just an annotation.
- **Q: What's the default bean scope, and why does it matter for thread safety?**
  A: Singleton -- one shared instance for the whole app. Concurrent requests hit the same instance, so mutable instance-field state causes race conditions; request-specific data must live in local variables, not fields.
- **Q: Difference between constructor and @PostConstruct?**
  A: The constructor runs during object creation, before dependencies are injected (for field/setter DI) -- injected fields may still be null inside it. @PostConstruct runs after all dependencies are injected, so it's the safe place for initialization logic that depends on them.
- **Q: What is a BeanPostProcessor, and where have you seen one used implicitly?**
  A: A hook that lets Spring inspect/modify beans before and after initialization. It's the mechanism Spring uses internally to wrap beans in proxies for @Transactional, @Async, @Cacheable, and @Scheduled -- which is why the object you actually hold a reference to is sometimes a proxy, not your raw class instance.
